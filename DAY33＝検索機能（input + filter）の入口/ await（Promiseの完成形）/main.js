// DAY33＝検索機能（input + filter）の入口だけ

// 🎯 今日のゴール（これだけ）

// input の値で一覧がリアルタイムに絞られる
...
// input が変化した結果として、イベント情報(e)が自動で渡される
...
// 入力された値 など
    ...
    // 👉 今 input に入っている文字列を取得↓
    // ここで何が起きてる？

    // e → イベント情報
    // e.target → イベントが起きた input 要素そのもの
    // value → inputの中身（文字）を取り出している
    ...
    // keyword を渡す
    // 中では searchPosts(keyword) が呼ばれる
    // 返ってきたデータだけを画面に表示

        renderSearchList(keyword);
    });

    // input が変わるとイベントが発生して
    // その情報が (e) として渡される
    // e.target は input 自体を指していて
    // value で中の文字を取り出す
    // 取り出した文字を keyword に入れて
    // renderSearchList(keyword) に渡している
